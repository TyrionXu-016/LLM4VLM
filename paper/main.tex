...
\paragraph{Comparison with VLN-BERT Baseline.}
As shown in Table~\ref{tab:main-results}, our model trained on LLM-generated Chinese instructions achieves competitive performance.
...
\midrule
VLN-BERT (paper) & R2R English & $\sim$55\% & $\sim$50\% & -- \\
\textbf{Ours} & LLM Chinese & \textbf{62.0\%} & \textbf{61.8\%} & \textbf{69.0\%} \\
\bottomrule
\end{tabular}
\caption{Comparison with VLN-BERT baseline. Our model achieves higher success rate on Chinese instructions.}
...
\begin{itemize}
    \item \textbf{Simulated data}: Visual features are synthetically generated, not from real R2R images.
    \item \textbf{Simplified evaluation}: We use action prediction accuracy rather than full navigation simulation.
    \item \textbf{Single language pair}: Only English$\rightarrow$Chinese transfer is studied.
    \item \textbf{Single-view features}: No temporal context integration.
    \item \textbf{Limited LLM comparison}: Primarily used Qwen models; broader comparison would be informative.
\end{itemize}
